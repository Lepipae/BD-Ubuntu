...
-- A ver yo creo que si que si hacer un day() + 2 o algo asi se puede jaja
...
-- 1.Obtén un listado con el nombre de cada cliente y el nombre y apellido de su
...
SELECT c.nombre_cliente, e.nombre, o.ciudad FROM oficina o
JOIN empleado e ON o.codigo_oficina = e.codigo_oficina
JOIN cliente c ON e.codigo_empleado = c.codigo_empleado_rep_ventas;
...
-- 9.Devuelve un listado que muestre el nombre de cada empleados, el nombre de su
-- jefe y el nombre del jefe de sus jefe.
SELECT e.nombre, j.nombre,j2.nombre FROM empleado e
JOIN empleado j ON e.codigo_jefe = j.codigo_empleado
JOIN empleado j2 ON j.codigo_jefe = j2.codigo_empleado;
-- 10.Devuelve el nombre de los clientes a los que no se les ha entregado a tiempo un
-- pedido.
...
-- cliente.
SELECT DISTINCT c.nombre_cliente, p.gama FROM cliente c
